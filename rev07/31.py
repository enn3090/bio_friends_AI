...
- 추가: Gradio 기반 GUI 제공
- 변경: OpenAI 함수/툴 바인딩 의존 제거
       → LLM이 JSON 계획을 출력하도록 유도하고, 파이썬에서 tools(retrieve, web_search, add_web_pages_json_to_chroma) 직접 호출
- 가정: 로컬 PC에 Ollama와 deepseek-r1 모델이 설치되어 있음 (예: `ollama pull deepseek-r1:latest`)
...
def render_references(refs: Dict[str, Any]) -> str:
    ...
    if docs:
        ...
        for i, d in enumerate(docs[:8], 1):
            ...
            lines.append(f"**{i}.** {preview[:160].replace('\n',' ')}" + (f"\n— _{src}_" if src else ""))
    return "\n\n".join(lines) or "(참조 없음)"
